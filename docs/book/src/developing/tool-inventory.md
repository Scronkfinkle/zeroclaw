...
| `caldav` | Plugin or MCP server. | Calendar read/write is tied to an external account and its credentials rather than the baseline agent contract. It stays first-party while ZeroClaw owns the SSRF guard, the encrypted credential, the read/act split, and the per-action allowlist; a replacement must reproduce all four. |
